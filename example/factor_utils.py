...
def preprocess(factors):
    ...
    """
    factors = factors.groupby(level='datetime').apply(fill_nan).reset_index(level=0, drop=True) # 填充NAN，以截面的均值来填充nan
    factors = factors.groupby(level='datetime').apply(winsorize).reset_index(level=0, drop=True)  # 去极值，把分数为97.5%和2.5%之外的异常值替换成分位数值
    factors = factors.groupby(level='datetime').apply(standardize).reset_index(level=0, drop=True) # 标准化（减去均值除以方差）
...
def neutralize(factor_df, df_stock_basic, df_mv):
    ...
    def _generate_cross_sectional_residual(data):
        ...
        for _, X in data.groupby(level=0):
            ...
            """
            X = pd.concat([X, pd.get_dummies(X.pop("industry")).astype(int)], axis=1)
            """
...
def handle_finance_ttm(stock_codes,
    ...
    for stock_code in stock_codes:
        ...
        for the_date in trade_dates:
            ...
            df_factor = pd.concat([df_factor, new_row], ignore_index=True)
