...
def load_stock_data(cerebro, start_date, end_date, stock_codes, atr_period):
    ...
    #先从datasource中读取股票数据
    df_stock = datasource.daily(stock_codes, start_date, end_date)
    df_stock = comply_backtrader_data_format(df_stock)
    #然后按股票代码去拆分df
    for stock_code in stock_codes:
        df_stock_one = df_stock[df_stock['code'] == stock_code]
        if len(df_stock_one) / len(trade_days) < 0.6:
            logger.warning("股票[%s] 缺失交易日[%d/总%d]天，超40%%，忽略此股票",
                           stock_code, len(df_stock_one), len(trade_days))
            continue
        # 按照backtrader的要求格式，把数据捋好
        # df_stock_one = comply_backtrader_data_format(df_stock_one)

        # 增加一列ATR
        df_stock_one['atr'] = talib.ATR(df_stock_one.high.values, df_stock_one.low.values, df_stock_one.close.values,
